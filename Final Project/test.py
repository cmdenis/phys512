...
class particles:
    ...
    def many_particles(self):
        ...
        '''
        np.random.seed(515) # Use a seed for repeatability
...
parts=particles(npart=2,n = 20, soft=2,periodic=True)
...
res=ax.imshow(parts.pot[:parts.ngrid,:parts.ngrid]**0.5)
...
for i in range(times):
    ...
    for j in range(osamp):
        ...
        res.set_data(parts.pot[:parts.ngrid, :parts.ngrid])
        print("Forces:", parts.f)
        plt.show()
        assert(0==1)
